...
export async function GET() {
  const meetings = (await getAllMeetings()).map(({ host_token: _, ...safe }) => safe);
  return Response.json(meetings);
}

export async function POST(request: NextRequest) {
  const body = await request.json();
  const { title, description, location, host_name, deadline, expected_attendees, candidate_dates } = body;
